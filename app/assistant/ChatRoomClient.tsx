...
export default function ChatRoomClient() {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: "Bonjour, je suis Jean-Francois. Posez-moi vos questions sur vos douleurs ou votre besoin de soin. Comment puis-je vous aider aujourd'hui ?" }
  ])
  ...
  useEffect(() => {
    const initChat = async () => {
      ...
      chatInstance.current = ai.chats.create({
        ...
        history: [],
        config: {
          systemInstruction: "Tu es Jean-Francois, un magnetiseur guerisseur bienveillant base a Alencon. Ton ton est calme. Tu aides pour le zona, l'eczema et les douleurs de dos. Si on demande ou tu te trouves, precise que tu es au Centre-ville d'Alencon (61000).",
          tools: [{ googleSearch: {} }],
        },
      })
    }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: 'model',
        text: "Le lien energetique est perturbe. Verifiez votre connexion.",
        isError: true
      }])
    } finally {
      setLoading(false)
