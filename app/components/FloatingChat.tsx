...
import { MessageCircle, X, Send, Globe, ExternalLink } from 'lucide-react'
...
export default function FloatingChat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: "Bonjour ! Je suis l'assistant de Jean-Francois. Je peux vous renseigner sur nos soins a distance partout en France ou sur notre cabinet a Alencon. Que puis-je faire pour vous ?" }
  ])
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction: "Tu es l'assistant de Jean-Francois, magnetiseur a Alencon specialise dans les soins a distance sur photo. Ton ton est rassurant et expert. Tu dois expliquer que la distance n'a aucune importance pour le magnetisme. Utilise Google Search pour trouver des explications sur les bienfaits du magnetisme ou des etudes si l'utilisateur est sceptique. Precise bien que Jean-Francois travaille pour toute la France.",
          tools: [{ googleSearch: {} }]
        },
      })
      ...
      setMessages(prev => [...prev, { role: 'model', text: "Une petite coupure d'energie... reessayez." }])
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="fixed bottom-24 right-4 md:bottom-6 md:right-6 z-[999] flex flex-col items-end pointer-events-none">
      {isOpen && (
        <div className="mb-4 w-[calc(100vw-32px)] sm:w-[400px] h-[550px] bg-white rounded-[2.5rem] shadow-2xl border border-stone-100 flex flex-col overflow-hidden pointer-events-auto">
          <div className="p-6 bg-indigo-600 text-white flex justify-between items-center">
            <div className="flex items-center gap-3">
              <Globe size={18} className="animate-pulse" />
              <div>
                <p className="font-serif font-bold text-sm">Assistant Jean-Francois</p>
                <p className="text-[9px] uppercase tracking-widest font-bold opacity-70">Expertise France Entiere</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-stone-50" ref={scrollRef}>
                  ...
                  {m.links && (
                    <div className="flex flex-wrap gap-2">
                      {m.links.map((link, idx) => (
                        <a key={idx} href={link.uri} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-stone-200 rounded-full text-[9px] font-bold text-indigo-600 hover:bg-indigo-50 shadow-sm">
            ...
            {loading && <div className="text-stone-400 text-[10px] animate-pulse italic">Jean-Francois reflechit...</div>}
          </div>

          <div className="p-4 bg-white border-t border-stone-100">
            <div className="flex gap-2 bg-stone-50 p-1 rounded-full border border-stone-200">
              <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Votre question sur le soin a distance..." className="flex-1 bg-transparent px-4 py-3 outline-none text-xs" />
