...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab }) => {
  ...
  const scrollToNav = () => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
    if (window.innerWidth < 1280) setIsMenuOpen(true);
  };
    ...
    <div className="min-h-screen flex flex-col">
      <div className="bg-stone-900 text-white/70 py-2.5 px-6 text-[10px] font-bold uppercase tracking-[0.2em] flex flex-col lg:flex-row justify-between items-center gap-4 z-[60] border-b border-white/5">
        ...
        <div className="flex items-center gap-6">
          <button onClick={scrollToNav} className="flex items-center gap-2 px-4 py-1.5 bg-white/5 rounded-full hover:bg-indigo-600 hover:text-white transition-all text-white border border-white/10 group">
      ...
      <header className="fixed top-0 lg:top-12 w-full z-50 bg-white/90 backdrop-blur-xl border-b border-stone-100 shadow-sm">
        <div className="max-w-[1600px] mx-auto px-6 h-20 flex items-center justify-between gap-8">
          <div className="flex items-center gap-5 cursor-pointer group shrink-0" onClick={() => setActiveTab('home')}>
            <div className="flex flex-col">
              <span className="text-2xl font-serif font-bold tracking-tight text-stone-900 group-hover:text-indigo-600 transition-colors leading-none">Jean-François</span>
              <span className="text-[10px] uppercase tracking-[0.2em] font-bold text-indigo-500 flex items-center gap-1 mt-1">Magnétiseur Guérisseur</span>
            </div>
          </div>
          ...
          <nav className="hidden xl:flex items-center gap-1 justify-center flex-1">
            {navItems.map((item) => (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all relative group ${activeTab === item.id ? 'text-indigo-600 bg-indigo-50/50' : 'text-stone-500 hover:text-stone-900 hover:bg-stone-50'}`}
              >
                <item.icon size={14} className={activeTab === item.id ? 'text-indigo-600' : 'text-stone-300 group-hover:text-stone-500'} />
                <span className="text-[11px] font-bold uppercase tracking-wider">{item.label}</span>
                {activeTab === item.id && <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-0.5 bg-indigo-600 rounded-full"></span>}
              </button>
            ))}
          </nav>

          <div className="flex items-center gap-4 shrink-0">
            <a href="tel:0955554462" className="hidden lg:flex items-center gap-2 px-5 py-3 bg-stone-900 text-white rounded-full text-[11px] font-bold hover:bg-black transition-all shadow-lg shadow-stone-200">
          ...
          <div className="xl:hidden absolute top-20 left-0 w-full bg-white border-b border-stone-100 p-6 space-y-4 shadow-2xl animate-in slide-in-from-top duration-300 z-50">
            {navItems.map((item) => (
              <button
                ...
                className={`flex items-center gap-4 w-full p-4 rounded-2xl text-base font-bold transition-all ${activeTab === item.id ? 'bg-indigo-50 text-indigo-600 shadow-inner' : 'text-stone-600 active:bg-stone-50'}`}
              >
                <div className={`p-2 rounded-lg ${activeTab === item.id ? 'bg-white text-indigo-600' : 'bg-stone-50 text-stone-400'}`}><item.icon size={20} /></div>
                {item.label}
              </button>
            ))}
          </div>
      ...
      <main className="flex-1 pt-20 lg:pt-40 pb-20 md:pb-0">{children}</main>
      ...
      <footer className="bg-stone-950 text-white py-20 px-6 mt-auto">
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-12">
          ...
          <div className="space-y-4">
            ...
            <p className="text-stone-500 text-sm italic">Le magnétisme ne se substitue pas à un avis médical.</p>
            <div onClick={handleSecretClick} className="pt-4 text-[10px] text-stone-600 uppercase tracking-widest font-bold cursor-pointer hover:text-indigo-500 transition-colors">
               Version 3.1 (Secured Admin)
            </div>
          </div>
        </div>
      </footer>
